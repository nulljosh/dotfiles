...
9. **Close** — Joshua closes Claude Code at the end of every session, so the checkpoint does it for him. This is the MAIN session's job, never the subagent's, and it is always the very last thing: only after the subagent has reported, the TLDR has been relayed, everything is pushed, and the loop handoff is written. Then run `kill -TERM $(ps -o ppid= -p $PPID | tr -d ' ')` style, walking up from the shell to the `claude` process (confirm the target with `ps -o comm= -p <pid>` and only kill if it is `claude`). Closing cancels any armed `/loop` wakeups and harness-tracked background tasks, which is fine because the handoff file holds the restart prompt. Things started with `nohup` (a training run) survive. Do NOT close if Joshua said to keep going, or if a subagent is still running. Before killing, run `git -C ~/Documents/Code/journal status --porcelain` and confirm the subagent's report actually came back: a dirty journal means the wrap died mid-commit, so finish it first (killed a half-done delta wrap 2026-09-25).
