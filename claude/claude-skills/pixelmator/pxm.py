...
def validate_spec(spec):
    ...
    for i, raw in enumerate(layers):
        ...
        if not isinstance(raw, dict):
            ...
            continue
        kind = raw.get("type")
        if kind != "text" and kind not in SHAPES:
            errs.append("%stype: expected one of %s, got %r"
                        % (where, ", ".join(sorted(list(SHAPES) + ["text"])), kind))
            continue
        allowed = TEXT_KEYS if kind == "text" else SHAPE_KEYS | EXTRA_KEYS.get(kind, set())
        ...
        layer = {
            "type": kind,
            "name": raw.get("name"),
            "x": coord(raw, "x", where, "center"),
            "y": coord(raw, "y", where, "center"),
...
def build_script(spec, timeout=120, headless=False, frames_dir=None):
    ...
    for i, L in enumerate(layers):
        ...
        x = "(%d - (width of L)) / 2" % W if L["x"] == "center" else _n(L["x"])
        y = "(%d - (height of L)) / 2" % H if L["y"] == "center" else _n(L["y"])
...
def make_gif(frames_dir, out_path, width, height, seconds=2.0):
    ...
    graph = ("color=white:s=%dx%d[bg];[bg][0:v]overlay=shortest=1,"
             ...
             "split[a][b];[a]palettegen[p];[b][p]paletteuse" % (width, height, min(width, 800)))
