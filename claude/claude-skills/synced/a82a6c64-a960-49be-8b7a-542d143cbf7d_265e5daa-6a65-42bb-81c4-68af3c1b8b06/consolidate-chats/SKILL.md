...
description: "Use when the user wants to reduce clutter in their claude.ai chat list by merging fragmented/duplicate-topic chats into one clean chat per topic via browser automation."
---

# Consolidate Chats

Goal: reduce chat-list clutter on claude.ai by merging chats that cover the same real topic into one clean chat per topic, without losing information, using browser automation tools.

## Process

1. **Inventory.** Open "Chats and tasks" (`claude.ai/chats`). List every chat title and last-updated date. Use `get_page_text` and scroll/load-more as needed since the list is virtualized. Distinguish "Chat" items from "Task" items — never touch Task items (they represent active agentic work).
...
3. **Extract per group (2+ chats).** For each chat in a group:
   - Read the full message history. claude.ai's message list is virtualized/windowed — clicking "Load earlier messages" can garbage-collect distant messages from the DOM, so call `get_page_text` again after each load rather than assuming everything stays loaded.
...
- Don't put "claude" in a skill's own name — that namespace reads as reserved for official Anthropic-authored skills.