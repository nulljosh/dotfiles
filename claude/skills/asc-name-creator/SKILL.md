...
- **Repo docs**: `grep -rl "<OLD_NAME>" <repo>` (excluding `.git/`, `.asc/artifacts/`), sed-replace across README, AGENTS.md, roadmap.md, and any web `index.html` / `manifest.json` (`name`, `short_name`, `<title>`, on-page brand strings).
- **Machine-wide docs**: `~/Documents/Code/AGENTS.md` reference table row.
