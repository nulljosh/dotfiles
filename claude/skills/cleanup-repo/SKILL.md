...
2. **Duplicate/nested copies.** Look for a subdirectory containing what looks like a second copy of the repo itself (README.md, index.html, AGENTS.md, roadmap.md all present one level down) — usually the result of an old `git subtree`/merge gone wrong. Grep the rest of the repo (docs, code, CI) for any reference to that path before deleting. If nothing references it, it's dead weight.
...
- If cleanup touches something the `~/.Codex/projects/-Users-joshua/memory/` files describe, note it, but memory updates aren't this skill's job.
