...
`~/.Codex/skills/landing-demo/inject.py` does the whole thing. Add a row to `APPS`:
...
Then run `python3 ~/.Codex/skills/landing-demo/inject.py`, screenshot once with headless Chrome (a Mac UA and an iPhone UA) to eyeball it, commit, push, and deploy with the repo's own deploy command. A push deploys nothing.
...
- No emojis, no gradients, house tokens only. See `~/Documents/Code/AGENTS.md`.
