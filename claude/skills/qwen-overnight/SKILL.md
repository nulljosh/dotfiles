...
## Usage awareness
This is already the zero-Codex-usage path (qwen2.5-coder via opencode does the work, not the Codex session) — that's the point of running it overnight instead of live. Keep it that way: don't add Codex subagent calls into the per-task loop, and check on progress via the log file, not by re-running the loop live.
