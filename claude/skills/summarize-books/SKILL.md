...
- Hook line missing → run `~/.Codex/scripts/usage.sh` before each new chapter.
...
- Don't use `summarize.sh` — it spawns `Codex -p` subprocesses that burn session tokens waiting. Read tool is faster and handles iCloud-evicted files correctly.
